...
def copy_state_dict(model, state_dict):
    ...
    print(f"Total matches: {direct_matches + transformer_matches + pretransform_matches}")
...
def copy_state_dict_regular(model, state_dict):
    ...
    for key in state_dict:
        if key in model_state_dict and state_dict[key].shape == model_state_dict[key].shape:
            ...
            changed_key =  key.replace('diffusion.', '', 1)  # Replace only the first occurrence
            if changed_key in model_state_dict and state_dict[key].shape == model_state_dict[changed_key].shape:
                if isinstance(state_dict[key], torch.nn.Parameter):
                    # backwards compatibility for serialized parameters
                    state_dict[key] = state_dict[key].data
