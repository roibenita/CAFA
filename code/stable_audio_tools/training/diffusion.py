...
class AVClipControlledDiffusionCondTrainingWrapper(pl.LightningModule):
    '''
    Wrapper for training a conditional audio diffusion model.
    '''
    def __init__(
        self,
        model: AVClipControlledConditionedDiffusionModelWrapper,
        lr: float = None,
        mask_padding: bool = False,
        mask_padding_dropout: float = 0.0,
        use_ema: bool = True,
...
class AVClipControlledDiffusionCondDemoCallback(pl.Callback):
    ...
    def __init__(
        ...
        sample_size=1024,
        demo_cfg_scales: tp.Optional[tp.List[int]] = [3, 5, 7],
        demo_cond_from_batch: bool = True,
        display_audio_cond: bool = False,
    ):
        super().__init__()
        self.demo_every = demo_every
    ...
    def on_train_batch_end(
        self,
        trainer,
        module: AVClipControlledDiffusionCondTrainingWrapper,
        outputs,
        batch,
        batch_idx,
    ):
        ...
        try:
            ...
            # Remove extra dimension added by WebDataset if present
            if reals.ndim == 4 and reals.shape[0] == 1:
                reals = reals[0]

            reals = reals[: self.num_demos].to(module.device)
            metadata = metadata[: self.num_demos]
            prompt = metadata[0]["prompt"]
            # Clean prompt for filename by removing special characters and limiting length
            ...
            # Log the original audio if requested
            if self.display_audio_cond:
                filename = f"demo_reals_{clean_prompt}_{trainer.global_step:08}.wav"
                audio_reals = rearrange(reals, "b d n -> d (b n)")
                audio_reals = (
                    audio_reals.to(torch.float32)
                    .div(torch.max(torch.abs(audio_reals)))
                    .mul(32767)
                    .to(torch.int16)
                    .cpu()
                )
                torchaudio.save(filename, audio_reals, self.sample_rate)
                log_dict[f"demo_reals"] = wandb.Audio(
                    filename, sample_rate=self.sample_rate, caption="Original Audio"
                )
                log_dict[f"demo_reals_melspec"] = wandb.Image(
                    audio_spectrogram_image(audio_reals)
                )
            ...
            for cfg_scale in self.demo_cfg_scales:
                ...
                filename = f"demo_{clean_prompt}_cfg_{cfg_scale}_{trainer.global_step:08}.wav"
                fakes = (
                ...
                log_dict[f"demo_cfg_{cfg_scale}"] = wandb.Audio(
                    filename,
                    sample_rate=self.sample_rate,
                    caption=f"Generated (CFG={cfg_scale})",
                )

                log_dict[f"demo_melspec_cfg_{cfg_scale}"] = wandb.Image(
