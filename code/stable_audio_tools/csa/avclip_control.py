...
class AVClipControlledContinuousTransformer(ContinuousTransformer):
    ...
    def forward(
        ...
        control_strength=None,  # Add control_strength parameter
        **kwargs,
    ):
        # with torch.no_grad():
        ...
        # Use provided control_strength or default to self.control_strength
        strength = (
            control_strength if control_strength is not None else self.control_strength
        )
        strength = strength.to(x.device)

        strength = torch.ones_like(strength)
        strength[(len(strength) // 2) :] = 0.5
        # strength = strength / 2
    ...
    def forward(
        ...
        **kwargs,
        ...
        if cfg_scale != 1.0 and (
            ...
            control_strength = torch.cat(
                [torch.ones(x.shape[0]), torch.zeros(x.shape[0])], dim=0
            )

            batch_control_signal = torch.cat([control_signal, control_signal], dim=0)

            # null_control = torch.zeros_like(control_signal, device=control_signal.device)
            # batch_control_signal = torch.cat([control_signal, null_control], dim=0)
            ...
            if cross_attn_cond is not None:
                ...
                if negative_cross_attn_cond is not None:
                    # raise ValueError(
                    #     "Negative cross-attention conditioning is not supported"
                    # )
